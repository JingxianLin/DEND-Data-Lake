...
from pyspark.sql.functions import year, month, dayofmonth, hour, weekofyear, date_format, dayofweek, max, monotonically_increasing_id
...
def create_bucket(bucket_name, region='us-west-2', acl='public-read'):
    try:
        if region:
            s3_client = boto3.client('s3', region_name=region)
            location = {'LocationConstraint':region}
            s3_client.create_bucket(Bucket=bucket_name, CreateBucketConfiguration=location, ACL=acl)
        else:
...
def main():
    ...
    output_data = "s3a://" + 'dend-jxlin' + "/"
    
    # create_bucket('dend-jingxian')
